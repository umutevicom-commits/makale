#!/usr/bin/env python3
"""
TechCrunch makale scraper'ı — sadece belirlenen tarama aralığında (varsayılan:
O GÜN YAYINLANAN; TC_LOOKBACK_DAYS ile genişletilebilir, ör. 7 için
bugün + geçmiş 7 gün / haftalık çekim) yayınlanan yeni haberler için.

Tüm siteyi baştan taramaz: haber listeleme sayfasını (/latest/) en yeni
kayıttan başlayarak okur, TechCrunch makale URL'lerinin kendi içinde taşıdığı
tarihi (https://techcrunch.com/YYYY/MM/DD/slug/ kalıbı) kullanarak tarama
aralığının DIŞINDA kalan veya daha önce işlenmiş (kayıt defterinde "ok"
durumunda) bir öğeye ulaşır ulaşmaz taramayı durdurur (liste kronolojik
...
Kalıcı durum: data/registry.json — haber ID'sine (URL'deki
"/YYYY/MM/DD/<slug>/" kalıbındaki benzersiz slug) göre anahtarlanan, her
haberin durumunu (ok/failed), URL'sini, tarihini ve bir SHA-256 içerik
özetini (hash) tutan kalıcı kayıt defteri. Bu defter sayesinde: (1) daha önce
işlenen hiçbir haber tekrar çekilmez, (2) başarısız olan haberler bir sonraki
çalıştırmada otomatik tekrar denenir, (3) RSS feed'i, her haber için BİR KEZ
üretilip diskte önbelleklenen <item> XML parçacıkları birleştirilerek
oluşturulur; eski haberlerin HTML/XML içeriği her çalıştırmada yeniden
üretilmez, sadece yeni haberler için üretilir ve mevcut feed'e eklenir.
"""
...
NEWS_INDEX_URL = f"{BASE_URL}/latest/"
...
SITE_NAME = os.environ.get("SITE_NAME", "TechCrunch Türkçe")
RSS_ITEM_LIMIT = 60

# Görseller TechCrunch'ın kendi CDN'inden (techcrunch.com/wp-content/uploads)
# doğrudan hotlink'lenmek yerine indirilip data/images/<slug>/ altında
# SAKLANIR ve RSS'e bu yayınlanan (GitHub Pages) adresle konur. Bu iki sorunu
# birden çözer: (1) TechCrunch'ın olası hotlink/referrer engeli görselin RSS
...
# 7 = bugün + geçmiş 7 gün (haftalık çekim). TC_LOOKBACK_DAYS ortam
# değişkeniyle ayarlanır.
LOOKBACK_DAYS = max(1, int(os.environ.get("TC_LOOKBACK_DAYS", "1")))
...
# da otomatik büyür; TC_MAX_PAGES açıkça verilirse o değer esas alınır.
MAX_PAGES = int(os.environ.get("TC_MAX_PAGES", str(max(5, LOOKBACK_DAYS * 5))))
DELAY_BETWEEN_TRANSLATIONS = 0.5  # saniye
# Çeviri yapılıp yapılmayacağı (1=evet, 0=hayır)
ENABLE_TRANSLATION = os.environ.get("TC_TRANSLATE", "1") != "0"
...
# Haber ID'si her zaman TechCrunch'ın kendi URL kalıbındadır:
# https://techcrunch.com/YYYY/MM/DD/slug/ — yıl/ay/gün YAYIN TARİHİNİ,
# slug ise haberin benzersiz kimliğini (news_id olarak kullanılır) verir.
# Bu tek regex hem "bu bir makale linki mi?" sorusunu hem de tarihi/ID'yi
# tek seferde, sayfa düzeninden tamamen bağımsız şekilde çözer.
ARTICLE_URL_RE = re.compile(
    r"^https?://(?:www\.)?techcrunch\.com/(\d{4})/(\d{2})/(\d{2})/([a-z0-9-]+)/?(?:[?#].*)?$",
    re.I,
)
...
# karaktere çıkar. TC_TRANSLATE_EMAIL ortam değişkeni ile ayarlanabilir.
TRANSLATE_EMAIL = os.environ.get("TC_TRANSLATE_EMAIL") or None
...
class TranslationFailedError(Exception):
    """Bir metin, TRANSLATION_RETRIES kadar tekrar denendikten sonra da
    çevrilemediğinde fırlatılır. KULLANICI İSTEĞİ: çevirisi tamamlanamayan
    bir makale KESİNLİKLE yayınlanmamalıdır — bu yüzden burada eskisi gibi
    sessizce orijinal (İngilizce) metne DÜŞÜLMEZ. Bu istisna, translate_text()
    ve translate_article() içinde YAKALANMADAN process_new_article()'a kadar
    yükselir; oradaki genel try/except onu yakalar, save_article() hiç
    çağrılmaz (makale diske/RSS'e hiç yazılmaz) ve haber registry'de "failed"
    olarak işaretlenip bir sonraki çalıştırmada (2 saat sonra) otomatik
    tekrar denenir — script'in zaten var olan tekrar-deneme mekanizması
    (retry_failed) hiçbir ek değişiklik gerekmeden bunu üstlenir."""
...
def _translate_chunk(text: str) -> str:
    ...
    birkaç kez tekrar dener. TÜM denemeler tükendiğinde ARTIK orijinal metne
    SESSİZCE düşülmEZ (bu, çevrilemeyen bir makalenin yarı İngilizce yarı
    Türkçe şekilde yayınlanmasına yol açardı) — bunun yerine
    TranslationFailedError fırlatılır; bu, çağıran zinciri boyunca
    yakalanmadan process_new_article()'a kadar yükselip makalenin hiç
    kaydedilmemesini/yayınlanmamasını sağlar."""
    ...
    for attempt in range(1, TRANSLATION_RETRIES + 1):
        try:
            ...
            time.sleep(RETRY_BACKOFF_SECONDS * attempt)
    reason = f"son hata: {last_exc}" if last_exc else "MyMemory tekrar tekrar kota/hata uyarısı döndürdü"
    print(f"    [çeviri-hata] {TRANSLATION_RETRIES} denemeden sonra vazgeçildi ({reason}); makale YAYINLANMAYACAK.")
    raise TranslationFailedError(
        f"çevrilemedi ({TRANSLATION_RETRIES} deneme tükendi, {reason}): "
        f"{text[:60]!r}..."
    )
...
def translate_text(text: str) -> str:
    ...
    Daha önce çevrilmiş aynı metin varsa API'ye tekrar istek atmadan
    önbellekten döndürülür. ÇEVİRİ KESİN OLARAK BAŞARISIZ olursa (bkz.
    _translate_chunk) burada YAKALANMAZ/orijinal metne düşülmez —
    TranslationFailedError bilerek yukarı (translate_article ->
    process_new_article) fırlatılmaya bırakılır ki makale hiç
    yayınlanmasın."""
...
# Yazar linki her zaman /author/<slug>/ kalıbındadır (class adı değişse bile sabit kalır)
AUTHOR_LINK_RE = re.compile(r"/author/[a-z0-9-]+/?$", re.I)
# Etiket/konu linkleri her zaman /tag/<slug>/ veya /category/<slug>/ kalıbındadır
TAG_LINK_RE = re.compile(r"/(?:tag|category)/[a-z0-9-]+/?$", re.I)
...
STOP_HEADING_MARKERS = (
    "related articles", "reader comments", "recommended", "topics",
    "most popular", "ilgili haberler", "yorumlar",
)
...
# (TechCrunch, makale sonuna "we may earn a small commission" ibareli bir
# afiliate açıklaması ekler; bu gövde metninin bir parçası değildir)
DISCLOSURE_SNIPPETS = (
    "affiliate partners", "qualifying sales", "preferred source on google",
    "we may earn a small commission", "affiliate monetization standards",
)

# TechCrunch'ta GSMArena'daki gibi tek bir sabit "ilgili haberler" URL
# kalıbı yoktur — dahası, makale gövdesi İÇİNDE başka TechCrunch
# haberlerine verilen normal satır-içi (inline) linkler tamamen geçerli
# içeriktir (ör. "companies like Micron" gibi bir cümle içindeki link) ve
# bunları bir SINIR sanıp gövdeyi erken kesmek YANLIŞ olur. Bu yüzden
# TechCrunch için URL tabanlı bir sınır sinyali kullanılmaz (hiçbir zaman
# eşleşmeyen bir kalıp); "İlgili haberler/Popüler" kutuları bunun yerine
# BOUNDARY_EXACT_TEXTS (başlık metni) ve SKIP_ANCESTOR_HINTS (üst eleman
# class/id ipucu: "related", "recommended", "popular", "similar" vb.) ile
# yakalanır.
REL_ARTICLE_LINK_RE = re.compile(r"(?!)")

# TechCrunch makalelerinde herkese açık bir yorum sistemi yoktur; olası bir
# yorum/disqus widget'ı eklenirse yine de yakalansın diye genel bir kalıp
# bırakılır. Bunlar makale gövdesinin bir parçası değildir ama gövde
# taramasını durdurmaz; yalnızca o tekil eleman atlanır.
COMMENT_LINK_RE = re.compile(r"disqus\.com|#comments$", re.I)
# Sosyal paylaşım linkleri (TechCrunch: Facebook/Twitter/LinkedIn/Reddit/e-posta paylaş ikonları)
SHARE_LINK_RE = re.compile(
    r"facebook\.com/sharer|twitter\.com/intent|linkedin\.com/share|"
    r"reddit\.com/submit|^mailto:",
    re.I,
)
...
BOUNDARY_EXACT_TEXTS = {
    "related articles", "recommended", "reader comments", "topics",
    "most popular", "ilgili haberler", "yorumlar", "önerilenler",
}
...
NOISE_EXACT_TEXTS = {
    ...
    "sponsored", "sponsored content", "ads by playwire", "view bio",
    "topics",
}
NOISE_TEXT_PATTERNS = (
...
def parse_article_links(html: str) -> list[str]:
    """Haber listeleme sayfasından makale URL'lerini, sayfadaki SIRAYA (en
    yeniden en eskiye) göre çıkarır. Hem mutlak hem göreceli href'leri
    (fix_url ile mutlaklaştırarak) kabul eder; sadece TechCrunch'ın kendi
    tarihli makale kalıbıyla (ARTICLE_URL_RE) eşleşenler alınır — /author/,
    /tag/, /category/, /video/, /events/ gibi makale OLMAYAN linkler bu
    kalıba uymadığı için kendiliğinden elenir."""
    soup = BeautifulSoup(html, "html.parser")
    links: list[str] = []
    seen: set[str] = set()
    for a in soup.find_all("a", href=True):
        full = fix_url(a["href"], NEWS_INDEX_URL)
        if ARTICLE_URL_RE.match(full) and full not in seen:
            seen.add(full)
            links.append(full)
    return links
...
def parse_listing_items(html: str) -> list[tuple[str, str, str]]:
    ...
    eskiye) göre (news_id, url, date_str) üçlüleri olarak döndürür.
    TechCrunch'ta yayın tarihi ayrıca sayfa metninden aranmaz: her makale
    URL'si zaten https://techcrunch.com/YYYY/MM/DD/slug/ kalıbında olduğundan
    tarih doğrudan URL'den okunur (sayfa düzeninden tamamen bağımsız,
    hatasız bir kaynak). news_id olarak da URL'deki benzersiz slug kullanılır
    (GSMArena'daki gibi ayrı bir sayısal haber ID'si yoktur)."""
    items: list[tuple[str, str, str]] = []
    seen_ids: set[str] = set()
    for link in parse_article_links(html):
        match = ARTICLE_URL_RE.match(link)
        if not match:
            continue
        ...
        date_str = date(int(year), int(month), int(day)).strftime("%d %B %Y")
        items.append((news_id, link, date_str))
    return items
...
def parse_pagination(html: str) -> str | None:
    """Sonraki sayfa linkini döndürür, yoksa None. TechCrunch'ın /latest/
    listesinde sayfalama "Next" metinli bir linkle (/latest/page/N/'e gider)
    yapılır; class adına bağımlı olmadan, sadece bu sabit metne bakılır."""
    soup = BeautifulSoup(html, "html.parser")
    for a in soup.find_all("a", href=True):
        if a.get_text(strip=True) in ("Next", "İleri", "»", "►"):
            return fix_url(a["href"], NEWS_INDEX_URL)
    return None


def extract_domain_from_url(url: str) -> str:
    """URL'den domain kısmını çıkarır (ör. techcrunch.com)."""
...
def fix_url(src: str, page_url: str) -> str:
    ...
    domaini önüne eklendiğinden, "https://techcrunch.com//www.youtube.com/..."
    gibi ASLA açılamayacak, kırık bir URL üretiliyordu (tarayıcı bunu
    techcrunch.com'un bir alt yolu sanıp orada arıyor, orası da iframe
...
def parse_article(url: str, html: str) -> Article:
    ...
    # Başlık
    h1 = soup.find("h1")
    title = _clean_paragraph_text(h1) if h1 else slug.replace("-", " ").title()

    # Yazar — önce href="/author/<slug>/" linkini ara (class adından bağımsız,
    # sayfa düzeni değişse de kırılmaz). Bulamazsa meta tag'e düş.
    author = ""
    ...
    # Tarih — TechCrunch URL'si her zaman /YYYY/MM/DD/slug/ kalıbında olduğundan
    # tarih doğrudan URL'den okunur (sayfa metnindeki "6:00 AM PDT · August 29,
    # 2026" gibi görüntüleme biçiminden bağımsız, hatasız bir kaynak). URL
    # eşleşmezse (beklenmedik bir adres biçimi) meta etiketine düşülür.
    date_str = ""
    url_date_match = ARTICLE_URL_RE.match(url)
    if url_date_match:
        year, month, day, _slug = url_date_match.groups()
        date_str = date(int(year), int(month), int(day)).strftime("%d %B %Y")
    if not date_str:
...
def migrate_registry_from_existing_articles() -> dict:
    ...
    for json_path in OUTPUT_DIR.glob("*/article.json"):
        ...
        news_id = article.news_id or (
            ARTICLE_URL_RE.match(article.url).group(4)
            if ARTICLE_URL_RE.match(article.url)
            else article.slug
        )
...
def main() -> int:
    ...
    print(f"TechCrunch makale scraper'ı başlatılıyor ({window_desc} yeni haberleri)...")
